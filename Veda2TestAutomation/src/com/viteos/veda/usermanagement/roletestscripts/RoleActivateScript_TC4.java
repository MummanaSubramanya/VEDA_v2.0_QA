...
public class RoleActivateScript_TC4 {
	...
	public static void activate(){
		...
		for(int index=1 ; index<=mapModifyDetails.size() ; index++){
			...
			if(mapSearchRoleDetails.get("Operation").equalsIgnoreCase("Validate") || mapSearchRoleDetails.get("Operation").equalsIgnoreCase("Modify") || mapSearchRoleDetails.get("Operation").equalsIgnoreCase("DeActivate")){
				continue;
			}
			...
			if(mapSearchRoleDetails.get("Operation").equalsIgnoreCase("Activate")){
				...
				bStatus = NewUICommonFunctions.activatePortal(mapSearchRoleDetails.get("RoleCode"), "Role");
				
				if(bStatus && mapSearchRoleDetails.get("ExpectedResult").equalsIgnoreCase("Pass")){
					Reporting.logResults("Pass", "Activate Created Role", "Role Activated Successfully for Role Name "+mapSearchRoleDetails.get("RoleCode"));
					continue;
				}
				if(!bStatus && mapSearchRoleDetails.get("ExpectedResult").equalsIgnoreCase("Pass")){
					Reporting.logResults("Fail", "Activate Created Role", "Failed to Activate Role Name "+mapSearchRoleDetails.get("RoleCode") +" ERROR: "+Messages.errorMsg);
					continue;
				}
				if(!bStatus && mapSearchRoleDetails.get("ExpectedResult").equalsIgnoreCase("Fail")){
					Reporting.logResults("Pass", "Activate Created Role", "Negative Test Case Cannot Perform Role Operation. ERROR: "+Messages.errorMsg);
					continue;
				}
				if(bStatus && mapSearchRoleDetails.get("ExpectedResult").equalsIgnoreCase("Fail")){
					Reporting.logResults("Fail", "Activate Created Role", "Role Activated for Negative TestCase.");
					continue;
				}
