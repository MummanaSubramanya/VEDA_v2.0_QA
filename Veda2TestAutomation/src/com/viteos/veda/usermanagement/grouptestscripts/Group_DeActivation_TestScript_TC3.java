...
public class Group_DeActivation_TestScript_TC3 {
	...
	public static void deActivate(){
		...
		for(int index=1 ; index<=mapModifyDetails.size() ; index++){
			...
			if(mapSearchGroupDetails.get("Operation").equalsIgnoreCase("Validate") || mapSearchGroupDetails.get("Operation").equalsIgnoreCase("Modify") || mapSearchGroupDetails.get("Operation").equalsIgnoreCase("Activate")){
				continue;
			}
			...
			if(mapSearchGroupDetails.get("Operation").equalsIgnoreCase("DeActivate")){
				...
				bStatus = NewUICommonFunctions.deactivatePortal(mapSearchGroupDetails.get("Group Name"), "Group");
				
				if(bStatus && mapSearchGroupDetails.get("ExpectedResult").equalsIgnoreCase("Pass")){
					Reporting.logResults("Pass", "DeActivate Created Group", "Group DeActivated Successfully for Group Name "+mapSearchGroupDetails.get("Group Name"));
					continue;
				}
				if(!bStatus && mapSearchGroupDetails.get("ExpectedResult").equalsIgnoreCase("Pass")){
					Reporting.logResults("Fail", "DeActivate Created Group", "Failed to DeActivate Group Name "+mapSearchGroupDetails.get("Group Name") +" ERROR: "+Messages.errorMsg);
					continue;
				}
				if(!bStatus && mapSearchGroupDetails.get("ExpectedResult").equalsIgnoreCase("Fail")){
					Reporting.logResults("Pass", "DeActivate Created Group", "Negative Test Case Cannot Perform Group Operation. ERROR: "+Messages.errorMsg);
					continue;
				}
				if(bStatus && mapSearchGroupDetails.get("ExpectedResult").equalsIgnoreCase("Fail")){
					Reporting.logResults("Fail", "DeActivate Created Group", "Group DeActivated for Negative TestCase.");
					continue;
				}
