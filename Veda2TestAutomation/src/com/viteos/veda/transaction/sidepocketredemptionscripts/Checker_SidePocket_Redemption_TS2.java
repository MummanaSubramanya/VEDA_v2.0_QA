...
public class Checker_SidePocket_Redemption_TS2 {
	...
	public static void vefifyCheckerOperations(){
		...
		for (int index = 1; index <= mapAllSidePCKTRedemptionDetails.size() ; index++){
			...
			if(mapSDPCKTRedemptiontionDetails.get("ExpectedResults").equalsIgnoreCase("Fail") || (!mapSDPCKTRedemptiontionDetails.get("OperationType").equalsIgnoreCase("Save") && !mapSDPCKTRedemptiontionDetails.get("OperationType").equalsIgnoreCase("Send For Review"))){
				continue;
			}
			if(mapSDPCKTRedemptiontionDetails.get("CheckerOperations").equalsIgnoreCase("None") || mapSDPCKTRedemptiontionDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("None")){
				continue;
			}
