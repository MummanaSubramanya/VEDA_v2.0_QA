...
public class Checker_SidePocket_Subscription_TS2 {
	...
	public static void test(){
		try {
			...
			for(int index = 1;index <= mapAllSPSUBDetails.size();index++){
				...
				if (!bStatus) {
					Reporting.logResults("Fail", "Checker Open the Created Side Pocket Subscription Order", "Checker Failed to Open the Created Side Pocket Subscription Order. "+Messages.errorMsg);
					continue;
				}
				...
				if(bStatus && mapSPSUBDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("Pass")){
					...
					Reporting.logResults("Pass", "Perform TradeTypeSidePocket_SUB Checker Operation : '"+mapSPSUBDetails.get("CheckerOperations")+"'", "Performed TradeTypeSidePocket_SUB Checker Operation '"+mapSPSUBDetails.get("CheckerOperations")+"' Successfully with Transaction ID : "+mapSP_SUB_OrderDetailsXML.get("TransactionID"));
					continue;	
				}		
				if(bStatus && mapSPSUBDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("Fail")){				
					Reporting.logResults("Fail", "Perform TradeTypeSidePocket_SUB Checker Operation : '"+mapSPSUBDetails.get("CheckerOperations")+"'", "Performed Checker Operation '"+mapSPSUBDetails.get("CheckerOperations")+"' on TradeTypeSidePocket_SUB with Negative Test Data");
					continue;				
				}			
				if(!bStatus && mapSPSUBDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("Pass")){

					Reporting.logResults("Fail", "Perform TradeTypeSidePocket_SUB Checker Operation : '"+mapSPSUBDetails.get("CheckerOperations")+"'", "Failed to Perform TradeTypeSidePocket_SUB Checker Operation '"+mapSPSUBDetails.get("CheckerOperations")+"'.ERROR : "+Messages.errorMsg);
				...
				if(!bStatus && mapSPSUBDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("Fail")){				
					Reporting.logResults("Pass", "Perform TradeTypeSidePocket_SUB Checker Operation : '"+mapSPSUBDetails.get("CheckerOperations")+"'", "Negative Test Data Cannot Perform TradeTypeSidePocket_SUB Checker Operation '"+mapSPSUBDetails.get("CheckerOperations")+"' .ERROR : "+Messages.errorMsg);
					NewUICommonFunctions.handleAlert();
					continue;				
				}			
			}
