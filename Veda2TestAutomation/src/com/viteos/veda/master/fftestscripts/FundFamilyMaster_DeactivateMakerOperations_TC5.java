...
public class FundFamilyMaster_DeactivateMakerOperations_TC5 {
	...
	public static void testSearchFunctions(){
		try{
			...
			for(int index = 1;index <= mapAllFundFamilyDetails.size();index++){
				...
				if(mapFundFamilySearchDetails.get("Operation").equalsIgnoreCase("Validate") || mapFundFamilySearchDetails.get("Operation").equalsIgnoreCase("Modify") || mapFundFamilySearchDetails.get("Operation").equalsIgnoreCase("Activate")){
					continue;
				}
				...
				if(mapFundFamilySearchDetails.get("Operation").equalsIgnoreCase("DeActivate")){
					...
					bStatus = NewUICommonFunctions.deactivateMaster("Fund Family Name", mapFundFamilySearchDetails.get("FundFamilyName"), "FundFamily");
					if(!bStatus){
						Reporting.logResults("Fail","Validate Client has been deactivated", "Fund Family has not be deactivated. Error: "+Messages.errorMsg);
						continue;
					}
					Reporting.logResults("Pass","Validate Client has been deactivated", "Fund Family "+mapCreatedFundFamilyDetails.get("FundFamilyName")+" Has been deactivated by Maker");
