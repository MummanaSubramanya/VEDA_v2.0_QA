...
public class InvestorMaster_DeactivateMakerOperations_TS5 {
	...
	public static void testSearchFunctions(){
		try{
			...
			for(int index = 1;index <= mapAllInvestorGeneralDetails.size();index++){
				...
				if(mapSearchInvestorDetails.get("Operation").equalsIgnoreCase("Deactivate")){
					bStatus = NewUICommonFunctions.deactivateMaster("Investor Name", sInvestorName, "Investor");
					if(!bStatus){
						Reporting.logResults("Fail","Maker : Validate Investor deactivate request has been raised.", "Maker : Investor deactivate request has NOT raised. Error: "+Messages.errorMsg);
						continue;
					}
					Reporting.logResults("Pass","Maker : Validate Investor deactivate request has been raised.", "Maker : Investor deactivate request has been raised Successfully.");
					continue;
				}
			}			
		}
