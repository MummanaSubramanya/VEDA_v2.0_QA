...
public class InvestorMaster_DeactivateCheckerOperations_TS6 {
	...
	public static void testSearchFunctions(){
		try{
			...
			for(int index = 1;index <= mapAllInvestorGeneralDetails.size();index++){
				...
				if (mapSearchInvestorDetails.get("Operation") == null || !mapSearchInvestorDetails.get("Operation").equalsIgnoreCase("Deactivate")) {
					continue;
				}
				...
				if(mapSearchInvestorDetails.get("Operation").equalsIgnoreCase("Deactivate")){
				...
				if(sInvestorName.equalsIgnoreCase("") || sInvestorName.equalsIgnoreCase(" ")) {
					Reporting.logResults("Fail", "[ TEST DATA ISSUE : Test Data wasn't given properly InvestorName required for search and deactivate.]", "[ TEST DATA ISSUE : Test Data wasn't given properly InvestorName required for search and deactivate.]");
					continue;
				}
				...
				if(!bStatus){
					Reporting.logResults("Fail","Checker : Validate Investor deactivate request has been : "+mapSearchInvestorDetails.get("CheckerOperations"), "Checker : Investor deactivate request has not been raised. Error: "+Messages.errorMsg);
					continue;
				}
				Reporting.logResults("Pass","Checker : Validate Investor deactivate request has been : "+mapSearchInvestorDetails.get("CheckerOperations"), "Checker : Investor deactivate request has been raised successfully.");
