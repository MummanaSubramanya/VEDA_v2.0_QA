...
public class InvestorAccountMaster_DeactivateMakerOperations_TC3 {
	...
	public static void testSearchFunctions(){
		try{
			...
			for(int index = 1;index <= mapAllInvestorAccountDetails.size();index++){
				...
				if(mapModifyInvestoraccountDetails.get("Operation").equalsIgnoreCase("Validate") || mapModifyInvestoraccountDetails.get("Operation").equalsIgnoreCase("Modify")||mapModifyInvestoraccountDetails.get("Operation").equalsIgnoreCase("Activate")){
					continue;
				}
				...
				if(mapModifyInvestoraccountDetails.get("Operation").equalsIgnoreCase("DeActivate")){
					
					bStatus = TempFunctions.deActivateFeeTye("Investor Holder", createdExcelInvestorDetailsDetails.get("Investor Holder"), "Investor Account Id", createdExcelInvestorDetailsDetails.get("AccountID"), "InvestorAccount");
					if(!bStatus){
						Reporting.logResults("Fail","Validate InvestorAccount  has been Deactivated", "InvestorAccount  has not be Deactivated. Error: "+Messages.errorMsg);
						continue;
					}
					Reporting.logResults("Pass","Validate InvestorAccount r has been Deactivated", "InvestorAccount  has been Deactivated");
					continue;
				}
			}
		}
