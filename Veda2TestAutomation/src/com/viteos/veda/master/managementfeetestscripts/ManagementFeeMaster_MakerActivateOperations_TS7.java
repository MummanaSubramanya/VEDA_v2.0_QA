...
public class ManagementFeeMaster_MakerActivateOperations_TS7 {
	...
	public static void testSearchFunctions(){
		...
		for(int index = 1;index <= mapAllManagementFeeDetails.size();index++){
			...
			if(mapModifyManagementFeeDetails.get("Operation").equalsIgnoreCase("Activate")){
				bStatus = TempFunctions.activateFeeTye("Legal Entity Name", createdManagementFeeDetails.get("Legal Entity Name"),"Management Fee Id", createdManagementFeeDetails.get("ManagementFeeID"),"ManagementFee");
				if(!bStatus){
					Reporting.logResults("Fail","Maker : Validate Management Fee has been Activated", " Maker : Management Fee has not been Activated : "+createdManagementFeeDetails.get("ManagementFeeID")+". "+Messages.errorMsg);
					continue;
				}
				Reporting.logResults("Pass","Maker : Validate Management Fee has been Activated", " Maker : Management Fee Has been Activated : "+createdManagementFeeDetails.get("ManagementFeeID"));
				continue;
			}
		}
	}
