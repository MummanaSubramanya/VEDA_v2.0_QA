...
public class ManagementFeeMaster_CheckerDeactivateOperations_TS6 {
	...
	public static void testSearchFunctions(){
		...
		for(int index = 1;index <= mapAllManagementFeeDetails.size();index++){
			...
			if(mapModifyManagementFeeDetails.get("Operation").equalsIgnoreCase("DeActivate")){
				...
				if(!bStatus){
					Reporting.logResults("Fail","Checker : Validate Management Fee has been Deactivated", " Management Fee has not been Deactivated . "+Messages.errorMsg);
					continue;
				}
				Reporting.logResults("Pass","Checker : Validate Management Fee has been Deactivated", " Management Fee Has been Deactivated : "+createdManagementFeeDetails.get("ManagementFeeID"));
				continue;
			}
		}
	}
