...
public class FeeDistributionMaster_MakerDeactivateOperations_TC5 {
		...
		for(int index = 1;index <= mapAllFeeDistributionDetails.size();index++){
			...
			if(mapModifyFeeDistributionDetails.get("Operation").equalsIgnoreCase("DeActivate")){
				if(mapModifyFeeDistributionDetails.get("CheckerOperations").equalsIgnoreCase("Approve")){
					bStatus = TempFunctions.deActivateFeeTye("Legal Entity Name", createdFeeDistributionDetails.get("Legal Entity Name"), "Fee Distribution Id", createdFeeDistributionDetails.get("FeeDistributionID"), "FeeDist");
					if(!bStatus){
						Reporting.logResults("Fail","Maker : Validate Fee Distribution has been requested to Deactivate", " Maker : is unable to request Fee Distribution to be Deactivated : "+Messages.errorMsg);
						continue;
					}
					Reporting.logResults("Pass","Maker : Validate Fee Distribution Fee has been requested to Deactivate", " Maker : hss successfully raised request to Deactivate Fee Distribution with ID : "+createdFeeDistributionDetails.get("FeeDistributionID"));
					continue;
				}
			}
		}
