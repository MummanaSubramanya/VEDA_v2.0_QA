...
public class SeriesMaster_DeactivateMakerOperations_TC5 {	
		...
		try{
			...
			for(int index = 1;index <= mapAllSeriesDetails.size();index++){
				...
				if(mapSeriesSearchDetails.get("Operation").equalsIgnoreCase("DeActivate")){
					...
					bStatus = NewUICommonFunctions.deactivateMaster("Series Name", mapSeriesSearchDetails.get("SeriesName"), "Series");
					if(!bStatus){
						Reporting.logResults("Fail","Validate Series has been deactivated for Series : "+mapSeriesSearchDetails.get("SeriesName"), " Series has not be deactivated. Error: "+Messages.errorMsg);
						continue;
					}
					Reporting.logResults("Pass","Validate Series has been deactivated for Series : "+mapSeriesSearchDetails.get("SeriesName"), " eries Has been deactivated");
					continue;
				}
			}
		}
