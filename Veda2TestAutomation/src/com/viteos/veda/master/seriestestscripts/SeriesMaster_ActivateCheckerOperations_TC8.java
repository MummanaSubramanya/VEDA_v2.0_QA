...
public class SeriesMaster_ActivateCheckerOperations_TC8 {
		...
		try{
			...
			for(int index = 1;index <= mapAllSeriesDetails.size();index++){
				...
				if(mapSeriesSearchDetails.get("Operation").equalsIgnoreCase("Activate")){
					...
					Map<String, String> seriesDetails = ExcelUtils.readDataABasedOnCellName(Global.sSeriesTestDataFilePath,"SeriesTestData", mapSeriesSearchDetails.get("TestcaseNameRef"));
					...
					if(!namemodifystatus){
						mapSeriesSearchDetails.put("SeriesName", seriesDetails.get("SeriesName"));
					}
