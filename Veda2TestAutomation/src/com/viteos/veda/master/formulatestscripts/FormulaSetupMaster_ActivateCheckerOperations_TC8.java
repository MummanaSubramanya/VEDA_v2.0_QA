...
public class FormulaSetupMaster_ActivateCheckerOperations_TC8 {
	...
	public static void testCheckerOperations(){
		try{
			...
			for(int index = 1;index <= mapAllFormulaDetails.size();index++){
				...
				if(mapFormulaDetails.get("Operation").equalsIgnoreCase("Validate") || mapFormulaDetails.get("Operation").equalsIgnoreCase("Modify") || mapFormulaDetails.get("Operation").equalsIgnoreCase("DeActivate")){
					continue;
				}
				...
				if(mapFormulaDetails.get("Operation").equalsIgnoreCase("Activate")){
				//boolean FeeType = false;
					...
					if(bStatus && mapFormulaDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("Pass")){
						Reporting.logResults("Pass","Perform Checker Operation", "Successfully Performed Activeted Formula with  checker operations: "+mapFormulaDetails.get("CheckerOperations"));
					}

					if(!bStatus && mapFormulaDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("Pass")){
						Reporting.logResults("Fail","Perform Checker Operation", "Cannot Perform Activeted Formula with Checker Operations: "+mapFormulaDetails.get("CheckerOperations")+". Error: "+Messages.errorMsg);
						continue;
					}

					if(!bStatus && mapFormulaDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("Fail")){
						Reporting.logResults("Pass","Perform Checker Operation", "Negative testcase - Cannot Perform Activeted Formula with Checker Operations: "+mapFormulaDetails.get("CheckerOperations")+". Error: "+Messages.errorMsg);
					}
	...
	private static void verifyValuesinSearchPanel(Map<String, Map<String, String>> verifyFormula) {
		try{
			...
			for (Entry<String, Map<String, String>> test : verifyFormula.entrySet()) {
					...
					bStatus = NewUICommonFunctions.selectMenu("Fund Setup","Formula Setup");
					if(!bStatus){
