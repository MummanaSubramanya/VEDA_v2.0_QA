...
public class OpeningBalanceMaster_ActivateCheckerOperations_TS8 {
		...
		for (int index = 1; index <= mapAllOpeningBalanceDetails.size(); index++) {
			...
			if (mapOpeningBalanceDetails.get("Operation") !=null && !mapOpeningBalanceDetails.isEmpty()) {
				if (mapOpeningBalanceDetails.get("Operation").trim().equalsIgnoreCase("Deactivate")) {					
					...
					if (!bStatus) {
						Reporting.logResults("Fail", "Checker : perform operation : "+mapOpeningBalanceDetails.get("CheckerOperations")+" ,against Activation request of Opening Balance with Txn ID : "+mapXMLOpeningBalanceDetails.get("SearchID"), "Checker : Unable to perform operation : "+mapOpeningBalanceDetails.get("CheckerOperations")+" ,against Activation request on the Opening Balance with Txn ID : "+mapXMLOpeningBalanceDetails.get("SearchID"));
						continue;
					}
					Reporting.logResults("Pass", "Checker : perform operation : "+mapOpeningBalanceDetails.get("CheckerOperations")+" ,against Activation request of Opening Balance with Txn ID : "+mapXMLOpeningBalanceDetails.get("SearchID"), "Checker : Successfully performed operation : "+mapOpeningBalanceDetails.get("CheckerOperations")+" ,against Activation request on the Opening Balance with Txn ID : "+mapXMLOpeningBalanceDetails.get("SearchID"));
