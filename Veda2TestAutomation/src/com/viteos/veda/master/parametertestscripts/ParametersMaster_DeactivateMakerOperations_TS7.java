...
public class ParametersMaster_DeactivateMakerOperations_TS7 {
		...
		try{
			...
			for(int index = 1;index <= mapAllParameterDetails.size();index++){
				...
				if(mapModifyParameterDetails.get("Operation").equalsIgnoreCase("DeActivate")){
					bStatus = TempFunctions.deActivateFeeTye("Parameter Name", createdParameterDetails.get("ParameterName"),"Parameter Id", createdParameterDetails.get("parameterID"),"Parameter");
					if(!bStatus){
						Reporting.logResults("Fail","Validate Parameter  has been Deactivated", "Parameter  has not be Deactivated. Error: "+Messages.errorMsg);
						continue;
					}
					Reporting.logResults("Pass","Validate Parameter has been Deactivated", "Parameter has been Deactivated");
					continue;
				}
			}
		}
