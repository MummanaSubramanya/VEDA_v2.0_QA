...
public class ParametersMaster_MakerOperations_TS3 {
			...
			for(int index = 1;index <= mapAllParametersDetails.size();index++){
				...
				if(mapParameterDetails.get("ExpectedResults").equalsIgnoreCase("Pass") && bStatus){
					...
					if(mapParameterDetails.get("OperationType").equalsIgnoreCase("Save")){
						...
						XMLLibrary.writeParametersForCreatedRulesToXML(mapParameterDetails);
					}
				...
				if(mapParameterDetails.get("ExpectedResults").equalsIgnoreCase("Fail") && !bStatus){
					Reporting.logResults("Pass", "Perform Maker Operation: "+mapParameterDetails.get("OperationType"), "Negative Testcase - Cannot perform maker operations for the scenario with test data " + mapParameterDetails);
				}
