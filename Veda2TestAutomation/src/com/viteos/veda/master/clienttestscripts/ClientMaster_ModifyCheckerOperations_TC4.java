...
public class ClientMaster_ModifyCheckerOperations_TC4 {
	...
	public static void testCheckerOperations(){
		try{
			...
			for(int index = 1;index <= mapAllClientDetails.size();index++){
				...
				if(mapClientDetails.get("Operation").equalsIgnoreCase("Validate")|| mapClientDetails.get("Operation").equalsIgnoreCase("DeActivate") || mapClientDetails.get("Operation").equalsIgnoreCase("Activate")){
					continue;
				}
				
				if(mapClientDetails.get("ExpectedResults")!=null && mapClientDetails.get("ExpectedResults").equalsIgnoreCase("Fail")){
					continue;
				}
				
				if(mapClientDetails.get("OperationType")!=null && mapClientDetails.get("OperationType").equalsIgnoreCase("Cancel")){
					continue;
				}
				...
				Map<String, String> clientDetails = ExcelUtils.readDataABasedOnCellName(Global.sClientTestDataFilePath,"ClientTestData", mapClientDetails.get("TestcaseNameRef"));

				/*if(mapClientDetails.get("ClientName")==null){
					mapClientDetails.put("ClientName", clientDetails.get("ClientName"));
				}
				*/
				clientDetails.putAll(mapClientDetails);

				// Verify Modify Details After Modification
				if(mapClientDetails.get("Operation").equalsIgnoreCase("Modify")){
					bStatus = NewUICommonFunctions.performOperationsOnTable(dashboardMainDropdownToSelect.MASTERS, dashboardSubDropdownToSelect.NEW, clientDetails.get("ClientName"),clientDetails.get("CheckerOperations"));
					if(bStatus && clientDetails.get("CheckerOperations").equalsIgnoreCase("Approve")){
						
						NewUICommonFunctions.selectMenu("Fund Setup","Client");
						Map<String, Map<String, String>> tempmap = new HashMap<String, Map<String,String>>();
						tempmap.put("1", clientDetails);
						boolean bTestStatus = NewUICommonFunctions.verifyMasterDetails("Client Name", clientDetails.get("ClientName"), "ClientMaster",tempmap);
				...
				if(bStatus && mapClientDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("Fail")){
					Reporting.logResults("Fail","Perform Checker Operation", "Peformed Checker operations with negative testdata. Cleint Name: "+clientDetails.get("ClientName"));
					continue;
				}	

				if(mapClientDetails.get("CheckerOperations").equalsIgnoreCase("Approve") && bStatus){
					innerMap.put("Status", "active");
					innerMap.put("ClientName", clientDetails.get("ClientName"));
