...
public class LegalEntity_CheckerDeactivateOperations_TC9 {
	...
	public static void testSearchFunctions(){
		try{
			...
			for(int index = 1;index <= mapAllModifyLegalEntityDetails.size();index++){
				...
				//Read test data based on Deactivate operation with Reference To "TCID".
				...
				//Verify Operation type and read the respective LE Name if it is modified.
				
				if(mapLeagalEntitySearchDetails.get("Operation").equalsIgnoreCase("DeActivate")){
					if (mapModifiedLEDetailsTabData.get("Legal Entity Name") != null && mapModifiedMainTabData!=null && mapModifiedMainTabData.get("Operation").equalsIgnoreCase("Modify")) {
					...
					if(!bStatus){
						Reporting.logResults("Fail","Checker : Deactivating Legal Entity", "Checker : Legal Entity : "+mapLeagalEntitySearchDetails.get("Legal Entity Name")+" has not been Deactivated");
						continue;
					}
					Reporting.logResults("Pass","Checker : Validate Legal Entity has been deactivated", "Checker : Legal Entity : "+mapLeagalEntitySearchDetails.get("Legal Entity Name")+" Has been Deactivated");
		...
		try{
			...
			for (Entry<String, Map<String, String>> test : verifyLE.entrySet()) {
				...
				if(bStatus && test.getValue().get("Status").equalsIgnoreCase("inactive")){
					Reporting.logResults("Pass", "Verify Legal Entity Name shouldnot be in active state",test.getKey()+" Legal Entity Name is not active state");
					//continue;
				}
				if(!bStatus && test.getValue().get("Status").equalsIgnoreCase("inactive")){
					Reporting.logResults("Fail", "Verify Legal Entity Name shouldnot be in active state",test.getKey()+" Legal Entity Name is in active state");
				}

				if(!bStatus && test.getValue().get("Status").equalsIgnoreCase("active")){
					Reporting.logResults("Fail", "Verify Legal Entity Name should be in active state",test.getKey()+" Legal Entity Name is not in active state");
					//continue;
				}
				if(bStatus && test.getValue().get("Status").equalsIgnoreCase("active")){
					Reporting.logResults("Pass", "Verify Legal Entity Name should be in active state",test.getKey()+" Legal Entity Name is in active state");
