...
public class LegalEntityMaster_MakerOperations_TC1 {
	...
	public static void addNewLegalEntity(){
		...
		for(int index = 1;index <= mapAllLegalEntityDetails.size();index++){
			...
			if(mapLegalEntityDetails.get("ExpectedResults").equalsIgnoreCase("Pass") && !bStatus){
				Reporting.logResults("Fail", "Perform Maker Operation: "+mapLegalEntityDetails.get("OperationType"), "Cannot perform maker Operations. Failed for the Legal Entity " + mapLegalEntityDetails.get("Legal Entity Name")+".ERROR: "+Messages.errorMsg);
				continue;
			}
			
			if(mapLegalEntityDetails.get("ExpectedResults").equalsIgnoreCase("Fail") && !bStatus){
				Reporting.logResults("Pass", "Perform Maker Operation: "+mapLegalEntityDetails.get("OperationType"), "Negative Testcase - Cannot perform maker operations for Legal Entity " + mapLegalEntityDetails.get("Legal Entity Name")+".ERROR: "+Messages.errorMsg);
			}
