...
public class IncentiveFeeMaster_MakerActivateOperations_TS7 {
		...
		for(int index = 1;index <= mapAllIncentiveFeeDetails.size();index++){
			...
			if(mapModifyIncentiveFeeDetails.get("Operation").equalsIgnoreCase("Activate")){
				if(createdIncentiveFeeDetails.get("CheckerOperations").equalsIgnoreCase("Approve")){
					bStatus = TempFunctions.activateFeeTye("Legal Entity Name", createdIncentiveFeeDetails.get("Legal Entity Name"), "Incentive Fee Id", createdIncentiveFeeDetails.get("IncentiveFeeID"), "IncentiveFee");
					//bStatus = IncentiveFeeAppFunctions.activateIncentiveFee("Legal Entity Name", createdIncentiveFeeDetails.get("Legal Entity Name"), "active", createdIncentiveFeeDetails.get("IncentiveFeeID"));
					if(!bStatus){
						Reporting.logResults("Fail","Maker : Validate Incentive Fee has been requested for Activation.", " Maker : failed to request for Incentive Fee Activation : "+Messages.errorMsg);
						continue;
					}
					Reporting.logResults("Pass","Maker : Validate Incentive Fee has been requested for Activation.", " Maker : successfully raised request to Activate Incentive Fee with ID : "+createdIncentiveFeeDetails.get("IncentiveFeeID"));
					continue;
				}
			}
		}
