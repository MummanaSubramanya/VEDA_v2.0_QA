...
public class HolderMaster_DeactivateMakerOperations_TS5 {
	...
	public static void testSearchFunctions(){
		try{
			...
			for(int index = 1;index <= mapAllHolderGeneralDetails.size();index++){
				...
				if(mapSearchHolderDetails.get("Operation").equalsIgnoreCase("Deactivate")){
					bStatus = NewUICommonFunctions.deactivateMaster("Investor Holder Name", sHolderName, "InvestorHolder");
					if(!bStatus){
						Reporting.logResults("Fail","Maker : Validate Holder deactivate request has been raised.", "Maker : Holder deactivate request has NOT raised. Error: "+Messages.errorMsg);
						continue;
					}
					Reporting.logResults("Pass","Maker : Validate Holder deactivate request has been raised.", "Maker : Holder deactivate request has been raised Successfully.");
					continue;
				}
			}			
		}
