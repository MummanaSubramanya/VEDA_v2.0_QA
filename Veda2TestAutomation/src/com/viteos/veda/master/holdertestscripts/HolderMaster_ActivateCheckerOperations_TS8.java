...
public class HolderMaster_ActivateCheckerOperations_TS8 {
	...
	public static void testSearchFunctions(){
		try{
			...
			for(int index = 1;index <= mapAllHolderGeneralDetails.size();index++){
				...
				if(!bStatus){
					Reporting.logResults("Fail","Checker : Validate Holder Activate request has been : "+mapSearchHolderDetails.get("CheckerOperations"), "Checker : Holder Activate request has NOT been : "+mapSearchHolderDetails.get("CheckerOperations")+". Error: "+Messages.errorMsg);
					continue;
				}
				Reporting.logResults("Pass","Checker : Validate Holder Activate request has been : "+mapSearchHolderDetails.get("CheckerOperations"), "Checker : Holder Activate request has been : "+mapSearchHolderDetails.get("CheckerOperations")+" Successfully.");
