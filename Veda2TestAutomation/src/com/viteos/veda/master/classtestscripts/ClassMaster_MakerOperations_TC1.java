...
public class ClassMaster_MakerOperations_TC1 {
	...
	@Test
	public static void addNewClassWithoutClone()
	{
		...
		for(int index = 1;index <= mapAllClassDetails.size();index++){
			...
			bStatus = ClassAppFunctions.addNewClass(objClassCreationTabsMaps);
			...
			if(mapClassDetails.get("ExpectedResults").equalsIgnoreCase("Pass") && bStatus){
				...
				if(mapClassDetails.get("OperationType").equalsIgnoreCase("Save")){
					Reporting.logResults("Pass", "Perform Maker Operation: "+mapClassDetails.get("OperationType"), "Performed maker Operation successfull Class Created Class Name : " + mapClassDetails.get("Class Name"));
				}
			...
			if(mapClassDetails.get("ExpectedResults").equalsIgnoreCase("Pass") && !bStatus){
				Reporting.logResults("Fail", "Perform Maker Operation: "+mapClassDetails.get("OperationType"), "Cannot perform maker Operations. Failed for the Class : " + mapClassDetails.get("Class Name")+".ERROR:"+Messages.errorMsg);
				continue;
			}
			
			if(mapClassDetails.get("ExpectedResults").equalsIgnoreCase("Fail") && !bStatus){
				Reporting.logResults("Pass", "Perform Maker Operation: "+mapClassDetails.get("OperationType"), "Negative Testcase - Cannot perform maker operations for Class Name : " + mapClassDetails.get("Class Name")+".ERROR:"+Messages.errorMsg);
			}
			...
			if(mapClassDetails.get("OperationType").equalsIgnoreCase("Save As Draft") && bStatus){
				...
				if (!bStatus) {
					Reporting.logResults("Fail","Verify Class Name in Saved As Draft Items", mapClassDetails.get("Class Name")+" Class Name is not visible in Saved as Draft Items");
					continue;
				}
				Reporting.logResults("Pass","Verify Class in Saved As Draft Items", mapClassDetails.get("Class Name")+" Class Name is visible in Saved as Draft Items");
				continue;
			}
		}
