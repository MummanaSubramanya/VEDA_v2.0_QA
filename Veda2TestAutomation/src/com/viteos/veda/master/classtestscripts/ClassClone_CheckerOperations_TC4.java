...
public class ClassClone_CheckerOperations_TC4 {
	...
	@Test
	public static void testClassCheckerOperationsWithClone(){
		try{
			...
			for(int index = 1;index <= mapAllClassDetails.size();index++){
				...
				if(bStatus && mapClassDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("Pass")){
					Reporting.logResults("Pass","Perform Checker Operation: "+mapClassDetails.get("CheckerOperations"), "Successfully Performed checker operations for Class: "+mapClassDetails.get("Class Name"));
				}

				if(!bStatus && mapClassDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("Pass")){
					Reporting.logResults("Fail","Perform Checker Operation: "+mapClassDetails.get("CheckerOperations"), "Cannot Perform Checker Operations. Error: "+Messages.errorMsg+". For Class: "+mapClassDetails.get("Class Name"));
					continue;
				}

				if(!bStatus && mapClassDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("Fail")){
					Reporting.logResults("Pass","Perform Checker Operation: "+mapClassDetails.get("CheckerOperations"), "Negative testcase - Cannot Perform Checker Operations for Class "+mapClassDetails.get("Class Name"));
				}

				if(bStatus && mapClassDetails.get("ExpectedResultsAfterCheckerOperations").equalsIgnoreCase("Fail")){
					Reporting.logResults("Fail","Perform Checker Operation: "+mapClassDetails.get("CheckerOperations"), "Peformed Checker operations with negative testdata. for Class Name: "+mapClassDetails.get("Class Name"));
					continue;
				}
